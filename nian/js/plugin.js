...
(function($) {
    ...
    $.fn.Chocolat = function(settings) {
        settings = $.extend({ container: $("body"), displayAsALink: false, linkImages: true, linksContainer: "Choco_links_container", overlayOpacity: 0.9, overlayColor: "#fff", fadeInOverlayduration: 100, fadeInImageduration: 100, fadeOutImageduration: 100, vache: true, separator1: " | ", separator2: "/", currentImage: 0, setIndex: 0, setTitle: "", lastImage: 0 }, settings);
        calls++;
        settings.setIndex = calls;
        images[settings.setIndex] = [];
        this.each(function(index) {
            if (index == 0 && settings.linkImages && settings.setTitle == "") { settings.setTitle = isSet($(this).attr("rel"), " ") }
            $(this).each(function() {
                images[settings.setIndex]["displayAsALink"] = settings.displayAsALink;
                images[settings.setIndex][index] = [];
                images[settings.setIndex][index]["adress"] = isSet($(this).attr("href"), " ");
                if (!settings.displayAsALink) { $(this).unbind("click").bind("click", { id: settings.setIndex, nom: settings.setTitle, i: index }, _initialise) }
            })
        });
        for (var i = 0; i < images[settings.setIndex].length; i++) {
            if (images[settings.setIndex]["displayAsALink"]) {
                if ($("#" + settings.linksContainer).size() == 0) { this.filter(":first").before('<ul id="' + settings.linksContainer + '"></ul>') }
                $("#" + settings.linksContainer).append('<li><a href="#" id="Choco_numsetIndex_' + settings.setIndex + '" class="Choco_link">' + settings.setTitle + "</a></li>");
                e = this.parent();
                $(this).remove();
                if ($.trim(e.html()) == "") { e.remove() }
        ...
        function _interface() {
            ...
            if (settings.vache) { $("#Choco_overlay").click(function() { close(); return false }) }
            $("#Choco_left_arrow").unbind().bind("click", function() { changePageChocolat(-1); return false });
            $("#Choco_right_arrow").unbind().bind("click", function() { changePageChocolat(1); return false });
            $(window).resize(function() { load(settings.currentImage, true) })
        }
        ...
        function changePageChocolat(signe) {
            if (!settings.linkImages || (settings.currentImage == 0 && signe == -1) || (settings.currentImage == settings.lastImage && signe == 1)) { return false } else {
                $("#Choco_container_description").css("visibility", "hidden");
                $("#Choco_bigImage").fadeTo(settings.fadeOutImageduration, 0, function() { load(settings.currentImage + parseInt(signe), false) })
            }
        }
        ...
        function ChoColat(hauteur_image, largeur_image, resize) {
            if (resize) {
                ...
                $("#Choco_bigImage").animate({ "height": hauteur_image + "px", "width": largeur_image + "px" }, settings.fadeInImageduration)
            }
            $("#Choco_container_photo").animate({ "height": hauteur_image, "width": largeur_image }, settings.fadeInImageduration);
            $("#Choco_content").animate({ "height": hauteur_image, "width": largeur_image, "marginLeft": -largeur_image / 2, "marginTop": -(hauteur_image) / 2 }, settings.fadeInImageduration, "swing", function() {
        ...
        function isSet(variable, defaultValue) { if (variable === undefined) { return defaultValue } else { return variable } }
            ...
            while (notFitting) {
                ...
                if (lblock > windowWidth) {
                    ...
                    if (hblock > windowHeight) {
                        ...
                        limg = k * himg
                    } else { notFitting = false }
                }
            }
            return { largeur: limg, hauteur: himg }
        }
...
! function(a) {
    a.fn.justifiedGallery = function(b) {
        function c(a, b, c) { var d; return d = a > b ? a : b, 100 >= d ? c.settings.sizeRangeSuffixes.lt100 : 240 >= d ? c.settings.sizeRangeSuffixes.lt240 : 320 >= d ? c.settings.sizeRangeSuffixes.lt320 : 500 >= d ? c.settings.sizeRangeSuffixes.lt500 : 640 >= d ? c.settings.sizeRangeSuffixes.lt640 : c.settings.sizeRangeSuffixes.lt1024 }

        function d(a, b) { return -1 !== a.indexOf(b, a.length - b.length) }

        function e(a, b) { return a.substring(0, a.length - b.length) }

        function f(a, b) {
            var c = !1;
            for (var e in b.settings.sizeRangeSuffixes)
                if (0 !== b.settings.sizeRangeSuffixes[e].length) { if (d(a, b.settings.sizeRangeSuffixes[e])) return b.settings.sizeRangeSuffixes[e] } else c = !0;
            if (c) return "";
        ...
        function l(a) { var b = a.find("> img"); return 0 === b.length && (b = a.find("> a > img")), b }
        ...
        function m(b, c, d, e, f, k, m) {
            ...
            if (m.settings.captions === !0) {
                ...
                if (0 === s.length) { var t = o.attr("alt"); "undefined" == typeof t && (t = b.attr("title")), "undefined" != typeof t && (s = a('<div class="caption">' + t + "</div>"), b.append(s)) }
        ...
        function n(a, b) {
            ...
            if (b && "hide" === i.lastRow && !o) { for (c = 0; c < a.buildingRow.entriesBuff.length; c++) d = a.buildingRow.entriesBuff[c], i.cssAnimation ? d.removeClass("entry-visible") : d.stop().fadeTo(0, 0); return -1 }
        ...
        function w(a) {
            function b(a) { if ("string" != typeof d.sizeRangeSuffixes[a]) throw "sizeRangeSuffixes." + a + " must be a string" }

            function c(a, b) { if ("string" == typeof a[b]) { if (a[b] = parseFloat(a[b], 10), isNaN(a[b])) throw "invalid number for " + b } else { if ("number" != typeof a[b]) throw b + " must be a number"; if (isNaN(a[b])) throw "invalid number for " + b } }
            var d = a.settings;
        ...
        return this.each(function(c, d) {
            ...
            if (w(f), f.entries = e.find("> a, > div:not(.spinner)").toArray(), 0 !== f.entries.length) {
                f.settings.randomize && (f.entries.sort(function() { return 2 * Math.random() - 1 }), a.each(f.entries, function() { a(this).appendTo(e) }));
                var m = !1,
                    n = !1;
                a.each(f.entries, function(b, c) {
                    ...
                    if (d.addClass("jg-entry"), g.data("jg.loaded") !== !0 && "skipped" !== g.data("jg.loaded")) {
                        ...
                        g.data("jg.loaded", !1), m = !0, f.spinner.active === !1 && (f.spinner.active = !0, e.append(f.spinner.$el), e.height(f.offY + f.spinner.$el.innerHeight()), r(f.spinner)), x(h, function(a) { g.data("jg.imgw", a.width), g.data("jg.imgh", a.height), g.data("jg.loaded", !0), u(f, !1) }, function() { g.data("jg.loaded", "error"), u(f, !1) })
                    }
                }), m || n || u(f, !1), q(f)
            }
        })
...
(function(c, e, f, b) {
    var i = "parallax";
    var g = 30;
    var d = { relativeInput: false, clipRelativeInput: false, calibrationThreshold: 100, calibrationDelay: 500, supportDelay: 1000, calibrateX: false, calibrateY: true, invertX: true, invertY: true, limitX: false, limitY: false, scalarX: 10, scalarY: 10, frictionX: 0.1, frictionY: 0.1, originX: 0.5, originY: 0.5 };
    ...
    function h(l, j) {
        ...
        for (var k in m) { if (m[k] === null) { delete m[k] } }
        c.extend(this, d, j, m);
    ...
    h.prototype.initialise = function() {
        if (this.$context.css("position") === "static") { this.$context.css({ position: "relative" }) }
    ...
    h.prototype.updateLayers = function() {
        ...
        this.$layers.each(c.proxy(function(j, k) { this.depths.push(c(k).data("depth") || 0) }, this))
    };
    ...
    h.prototype.disable = function() {
        if (this.enabled) {
            this.enabled = false;
            if (this.orientationSupport) { e.removeEventListener("deviceorientation", this.onDeviceOrientation) } else { e.removeEventListener("mousemove", this.onMouseMove) }
    ...
    h.prototype.css = function(m, p, o) {
        ...
        if (!n) { for (var k = 0, j = this.vendors.length; k < j; k++) { if (this.vendors[k] !== null) { n = c.camelCase(this.vendors[k][1] + "-" + p) } else { n = p } if (m.style[n] !== b) { this.propertyCache[p] = n; break } } }
        m.style[n] = o
    };
    ...
    h.prototype.setPosition = function(k, j, l) {
        j += "px";
        l += "px";
        if (this.transform3DSupport) { this.css(k, "transform", "translate3d(" + j + "," + l + ",0)") } else {
            if (this.transform2DSupport) { this.css(k, "transform", "translate(" + j + "," + l + ")") } else {
                k.style.left = j;
                k.style.top = l
            }
        }
    ...
    h.prototype.onCalibrationTimer = function(j) { this.calibrationFlag = true };
    h.prototype.onWindowResize = function(j) { this.updateDimensions() };
    h.prototype.onAnimationFrame = function() {
        this.updateBounds();
        var m = this.ix - this.cx;
        var k = this.iy - this.cy;
        if ((Math.abs(m) > this.calibrationThreshold) || (Math.abs(k) > this.calibrationThreshold)) { this.queueCalibration(0) }
        if (this.portrait) {
    ...
    c.fn[i] = function(k) {
        ...
        return this.each(function() {
            ...
            if (a[k]) { l[k].apply(l, Array.prototype.slice.call(j, 1)) }
        })
    }
...
(function e$$0(x, z, l) {
    ...
    1: [function(A, x, z) {
        if (!l) var l = {
            ...
            naturalOrder: function(h, g) { return h < g ? -1 : h > g ? 1 : 0 },
            sum: function(h, g) { var l = {}; return h.reduce(g ? function(h, b, a) { l.index = a; return h + g.call(l, b) } : function(h, b) { return h + b }, 0) },
            max: function(h, g) { return Math.max.apply(null, g ? l.map(h, g) : h) }
        };
        A = function() {
            ...
            function g(f) {
                ...
                return {
                    ...
                    pop: function() { b || c(); return a.pop() },
                    size: function() { return a.length },
                    map: function(c) { return a.map(c) },
                    debug: function() { b || c(); return a }
                }
            }
            ...
            var d = 5,
                e = 8 - d;
            w.prototype = {
                volume: function(a) { if (!this._volume || a) this._volume = (this.r2 - this.r1 + 1) * (this.g2 - this.g1 + 1) * (this.b2 - this.b1 + 1); return this._volume },
                count: function(a) {
                    var c = this.histo;
            ...
            p.prototype = {
                push: function(a) { this.vboxes.push({ vbox: a, color: a.avg() }) },
                palette: function() { return this.vboxes.map(function(a) { return a.color }) },
                ...
                forcebw: function() {
                    var a = this.vboxes;
                    a.sort(function(a, b) { return l.naturalOrder(l.sum(a.color), l.sum(b.color)) });
                    var b = a[0].color;
            ...
            return {
                quantize: function(d, c) {
                    ...
                    var r = a(d, h),
                        q = new g(function(a, b) { return l.naturalOrder(a.count(), b.count()) });
                    q.push(r);
                    e(q, 0.75 * c);
                    for (r = new g(function(a, b) { return l.naturalOrder(a.count() * a.volume(), b.count() * b.volume()) }); q.size();) r.push(q.pop());
                    e(r, c - r.size());
    ...
    2: [function(A, x, z) {
        (function() {
            var l,
                h, g, w = function(b, a) { return function() { return b.apply(a, arguments) } },
                p = [].slice;
            window.Swatch = h = function() {
                ...
                b.prototype._ensureTextColors = function() { if (!this.yiq) return this.yiq = (299 * this.rgb[0] + 587 * this.rgb[1] + 114 * this.rgb[2]) / 1E3 };
                return b
            }();
